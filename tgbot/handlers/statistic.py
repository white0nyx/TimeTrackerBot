...
from tgbot.misc.analytics import get_plot_total_time, get_diagram_week_statistic, get_circle_diagram_sessions_durations, \
    get_diagram_by_hours_in_day, get_circle_diagram_time_to_category
...
async def statistic_button(message: Message, state: FSMContext):
    ...
    # Диаграмма со статистикой по категориям
    get_circle_diagram_time_to_category(str(user_id), period_statistic_in_days)
    diagram_time_to_category = InputFile(f'data/{user_id}_diagram_time_to_category.png')
    album.attach_photo(diagram_time_to_category)
    ...
    os.remove(f'data/{user_id}_diagram_time_to_category.png')
